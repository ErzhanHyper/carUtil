...
//                        $kap_request = KapRequest::where('iinbin', $client->idnum)->orderByDesc('created_at')->first();
        ...
        if ($kap_request) {
            ...
            if ($kap_request->k_status === 'Проверка VIN, номер кузова не снят с учета (V004)') {
                $success = false;
                $result['message'] = $kap_request->k_status;
                $can = true;
                $value = $user->idnum;
                $type = 'iinbin';
            }
...
//        exec('cd /var/www/test && LD_LIBRARY_PATH="/opt/kalkancrypt:/opt/kalkancrypt/lib/engines" php list.php '.$vin, $result_data);
        ...
        if ($success_checking) {
            $kap_request_id = $this->storeKapRequest(new Request([
                'result_data' => $result_data,
                'vin' => $vin,
                'message' => $message
            ]));

            if (!empty($kap_request_id)) {
                return [
                    'id' => $kap_request_id,
                    'status' => true,
                    'message' => $message
...
//        exec('cd /var/www/test && LD_LIBRARY_PATH="/opt/kalkancrypt:/opt/kalkancrypt/lib/engines" php grnz_list.php '.$grnz, $result_data);
    ...
    public function storeKapRequest($request){
        $user = app(AuthService::class)->auth();
        ...
        if($request->vin) {
            $kap_request->vin = $request->vin;
        }else if($request->grnz){
           $kap_request->grnz = $request->grnz;
        }else if($request->iinbin){
            $kap_request->iinbin = $request->iinbin;
        }

        $kap_request->k_status = $request->message;
        $kap_request->xml_response = $request->result_data;
        $kap_request->user_id = $user_id;
        $kap_request->created_at = time();
        $kap_request->save();

        return $kap_request->id;
    }
}

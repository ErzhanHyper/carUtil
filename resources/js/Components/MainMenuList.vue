<template>
    ...
    <q-toolbar class="bg-primary text-white text-center ">
        <q-toolbar-title class="text-uppercase text-overline text-weight-bold">CarUtil</q-toolbar-title>
    </q-toolbar>
...
export default {
    ...
    data() {
        return {
            ...
            dealer_chief_menu: menu.dealerChiefItems,
            items: [],
            show: false
        }
    },
    ...
    created() {
        getUser({}).then(res => {
            if(res && res.role) {
                ...
                } else if (res.role === 'dealer-chief') {
                    this.items = this.dealer_chief_menu
                }
                this.show = true
            }
        })
